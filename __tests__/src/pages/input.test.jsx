...
describe("StyledInput", () => {
  ...
  it("renders input components with type password", () => {
    const { container } = render(<StyledInput type="password" variant="filled" label="Text" adorment={true} />);
    const input = container.getElementsByClassName('custom-input');
    expect(input).toBeTruthy();
  });

  it("renders input components with type dob", () => {
    const { container } = render(<StyledInput type="dob" variant="outlined" label="Dates" adorment={false} />);
    const input = container.getElementsByClassName('custom-input');
    expect(input).toBeTruthy();
  });
});
